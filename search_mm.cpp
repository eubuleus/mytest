...
#include "Vector.h"      //,    only: Dir,num_dir
#include "DS_Mol.h"      //, only: getAllNeigh
#include "Dam.h"      //,    only: n_dam, AddDamNode, DistanceToDam
#include "Common.h"      //,    only: len, E_cut, n_Minima, ix_Minima, NodeType, dE
#include "Search_Mol.h"      //,only: &
//          List_OnSurf , list_ToVisit , list_Visitin , n_OnSurf ,n_ToVisit ,n_Visitin, &
//          list_OnSurf2, n_OnSurf2,n_ToVisit2,n_Visitin2

void SearchMM(E_surf){
//! ______________________________________________________________________

    float,intent(inout):: E_surf
//! - index -
    integer:: ind(5,2), ix, ix1, ix2, ix_min        ! 
//! - 
    Node *pN, *pN1;
    int Ilrzcs(5),Il;    
    float cm12;
...
L0: while(true){                                    //! loop over different water levels
        E_surf += dE;        write(*,'(a,f10.3,a,i6)') 'E_surf =',E_surf, '   n_dam =',n_dam
...
L1:     while(true){                                //! visit all nodes between level L and L+1
            ...
            print*,'n_Visitin =',n_Visitin,n_dam_candi,E_surf
...
// ________________________  new barrier ?_______________________________
!
        n_dam_new = 0
LA:     for( n = 0;; n =){
...
L2:     while(true){                                // visit all nodes between level L and L+1
            for( i = 1,n_Visitin2){
                    ix = list_Visitin(i,2);
                    pN => Nodes(ix);
                    if( pN.wet_tag.b_Wet) stop 'debug error: in Search_MM: pN.b_Wet -------------****'
                    GetAllNeigh(ix,len,n_NBs,ix_NBs);
                    icount = 0
               // ! -
                    for ( k = 0; k < n_NBs; k++){        // loop over all neighbors of node ix
